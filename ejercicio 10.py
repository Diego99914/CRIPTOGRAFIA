import os
from Crypto.PublicKey import RSA
from Crypto.Hash import SHA256
from Crypto.Cipher import PKCS1_OAEP

#Indico de donde quiero que lea las claves 

my_path = os.path.dirname(file)
path_file_publ = my_path + "/clave-rsa-oaep-publ.pem"
path_file_priv = my_path + "/clave-rsa-oaep-priv.pem"

#Importo las claves gracias al RSA.import

keypub =RSA.import_key(open(path_file_publ).read())image.png
keypriv =RSA.import_key(open(path_file_priv).read())

#Aquí descifro usando de guía la muestra de como descifrar con RSA OAEP 

decryptor = PKCS1_OAEP.new(keypriv,SHA256)
textocifrado=bytes.fromhex('')
decrypted = decryptor.decrypt(textocifrado)

#Pido que me lo muestre en hexadecimal 

print('El texto descifrado en hexadecimal:', decrypted.hex())

#Lo vuelvo a cifrar como me pide el ejercicio 

cipher =  PKCS1_OAEP.new(keypub,SHA256)
encrypted = cipher.encrypt(decrypted) 

#De nuevo pido que lo enseñe en hexadecimal

print ('El texto que descifré , vuelto a cifrar : ',encrypted.hex())